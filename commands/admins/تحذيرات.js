    ...
    if (interaction.commandName == 'تحذير البائع') {
        ...
        collector.on('collect', async i => {
            ...
            if (warningType === 'تحذير') {
                ...
                if (guildMember.roles.cache.has(warn50)) {
                    guildMember.roles.add(warn100).catch(console.error);
                }
            
                if (guildMember.roles.cache.has(warn100)) {
                ...
                if (guildMember.roles.cache.has(warn25)) {
                    guildMember.roles.remove(warn25).catch(console.error);
                }
            
                if (guildMember.roles.cache.has(warn50)) {
                    guildMember.roles.remove(warn50).catch(console.error);
                }
            
                if (guildMember.roles.cache.has(warn100)) {
                    guildMember.roles.remove(warn100).catch(console.error);
                }
                    await RolesSellers.forEach(async roleId => {
                if (guildMember.roles.cache.has(roleId)) {
                    await guildMember.roles.remove(roleId).catch(console.error);
            ...
            const T = await Room.send({ embeds: [embed] });
